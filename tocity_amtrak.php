...
$csvFile = dirname(__FILE__) . '/amtrak_search.csv'; // Updated CSV file name
...
<?php
    ...
    foreach ($csvData as $index => $row) {
        ...
        // Ensure latitude and longitude columns exist and are numeric (column 8 for longitude, 9 for latitude)
        ...
        $stationLatitude = floatval($row[9]); // Convert Latitude column index to float (column 9)
        $stationLongitude = floatval($row[8]); // Convert Longitude column index to float (column 8)
