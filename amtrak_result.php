...
<head>
    <title>Amtrak Station Search</title>
    <style>
        table th {
    ...
    <?php 
    include 'fromcity_amtrak.php'; 
    include 'tocity_amtrak.php';
    error_log("Included fromcity_amtrak.php and tocity_amtrak.php successfully.");
    ?>
</head>

<body>

    <!-- From City Amtrak Station Table -->
    <div class="container_fromcity">
        <h2 class="from_station">From Amtrak Stations of
        ...
        if (empty($filteredData)) {
            error_log("No data found for From Amtrak Stations within 50 miles.");
            echo "<p>No data found for the given city and state within 50 miles.</p>";
        } else {
            error_log("Displaying From Amtrak Stations table.");
        ?>
            <table id="amtrak-from-table" class="table table-bordered table-striped">
                <thead>
                    <tr>
    ...
    <!-- To City Amtrak Station Table -->
    <div class="container_tocity">
        <h2 class="to_station">To Amtrak Stations of
            <span>(<?php echo htmlspecialchars($dataArray['toCity'] . ', ' . $dataArray['toState']); ?>)</span>
        </h2>

        <?php
        // Log filtered data check for To City Amtrak stations
        if (empty($tofilteredData)) {
            error_log("No data found for To Amtrak Stations within 50 miles.");
            echo "<p>No data found for the given city and state within 50 miles.</p>";
        } else {
            error_log("Displaying To Amtrak Stations table.");
        ?>
            <table id="amtrak-to-table" class="table table-bordered table-striped">
                <thead>
                    <tr>
                        <th data-sortable="true" data-field="station_name">
                            Station Name <span class="arrow"></span>
    ...
    <script>
        ...
        $(document).ready(function () {
            console.log("Initializing DataTable for From Amtrak Stations.");
            $('#amtrak-from-table').DataTable({
                "paging": false,
                "searching": false,
                "info": false,
            ...
            console.log("Initializing DataTable for To Amtrak Stations.");
            $('#amtrak-to-table').DataTable({
                "paging": false,
                "searching": false,
                "info": false,
                "order": [[4, "asc"]] // Sort by the 5th column (Distance) in ascending order
